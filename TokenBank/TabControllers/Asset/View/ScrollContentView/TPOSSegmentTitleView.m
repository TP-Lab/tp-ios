//
//  TNSegmentTitleView.m
//
//  Created by huim on 2017/5/3.
//  Copyright © 2017年 fengshun. All rights reserved.
//

#import "TPOSSegmentTitleView.h"

@interface TPOSSegmentTitleView ()<UIScrollViewDelegate>
...
@property (nonatomic, assign) TPOSIndicatorType indicatorType;

@property (nonatomic, strong) NSArray *titlesArr;

@end

@implementation TPOSSegmentTitleView
...
- (void)layoutSubviews
{
    ...
    if (_titleFont != _titleSelectFont) {
        for (int idx = 0; idx < self.titlesArr.count; idx++) {
            ...
            CGFloat itemBtnWidth = [TPOSSegmentTitleView getWidthWithString:self.titlesArr[idx] font:titleFont] + self.itemMargin;
    ...
    else
    {
        for (NSString *title in self.titlesArr) {
            CGFloat itemBtnWidth = [TPOSSegmentTitleView getWidthWithString:title font:titleFont] + self.itemMargin;
            totalBtnWidth += itemBtnWidth;
        }
    }
...
- (void)moveIndicatorView:(BOOL)animated
{
    ...
    CGFloat indicatorWidth = [TPOSSegmentTitleView getWidthWithString:self.titlesArr[self.selectIndex] font:titleFont];
    [UIView animateWithDuration:(animated?0.05:0) animations:^{
        switch (_indicatorType) {
            case TPOSIndicatorTypeDefault:
                ...
                break;
            case TPOSIndicatorTypeEqualTitle:
...
- (void)btnClick:(UIButton *)btn
{
    ...
    if (self.delegate&&[self.delegate respondsToSelector:@selector(TPOSSegmentTitleView:startIndex:endIndex:)]) {
        [self.delegate TPOSSegmentTitleView:self startIndex:self.selectIndex endIndex:index];
    }
...
{
    if (self.delegate&&[self.delegate respondsToSelector:@selector(TPOSSegmentTitleViewWillBeginDragging:)]) {
        [self.delegate TPOSSegmentTitleViewWillBeginDragging:self];
    }
}

- (void)scrollViewWillEndDragging:(UIScrollView *)scrollView withVelocity:(CGPoint)velocity targetContentOffset:(inout CGPoint *)targetContentOffset
{
    if (self.delegate&&[self.delegate respondsToSelector:@selector(TPOSSegmentTitleViewWillEndDragging:)]) {
        [self.delegate TPOSSegmentTitleViewWillEndDragging:self];
    }
}
